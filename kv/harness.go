...
// Test harness for server and client system tests.
type Harness struct {
	n int
...
func NewHarness(t *testing.T, n int) *Harness {
	...
	for i := range n {
		...
		//kvss[i] = server.New(i, peerIds, storage[i], ready)
		alive[i] = true
	}
...
func (h *Harness) RestartService(id int) {
	...
	ready := make(chan any)
	//h.kvCluster[id] = server.New(id, peerIds, h.storage[id], ready)
...
// services.
func (h *Harness) NewClient() *client.KVClient {
	var addrs []string
	for i := range h.n {
		if h.alive[i] {
